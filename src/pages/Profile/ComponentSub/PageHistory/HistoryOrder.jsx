    ...
    if (productOrderList) {
      ...
      return productOrderList.data.map((item, index) => {
        return {
          ...
          addressCustomer:
            item.consigneeInfor.wards +
            "-" +
            item.consigneeInfor.district +
            "-" +
            item.consigneeInfor.city,
          totalMoney: item.totalOrder.toLocaleString("vi", {
            style: "currency",
            currency: "VND",
          }),
          ...
          inforProduct: item.cart.map((itemCart, indexCart) => {
            return {
              key: indexCart + 1,
              nameProduct:
                Object.keys(itemCart.productOptions).length > 0
                  ? itemCart.name + " " + itemCart.productOptions.memory
                  : itemCart.name,
              imageProduct: (
              ...
              priceProduct:
                Object.keys(itemCart.productOptions).length > 0
                  ? (
                      itemCart.price + itemCart.productOptions.price
                    ).toLocaleString("vi", {
                      style: "currency",
                      currency: "VND",
                    })
                  : itemCart.price.toLocaleString("vi", {
                      style: "currency",
                      currency: "VND",
                    }),
            };
...
const mapDispatchToProps = (dispatch) => {
  return {
    getProductOrderList: (params) =>
      dispatch(getProductOrderListAction(params)),
  };
};
